...
; (function($) {
	...
	$.fn.TagTagger = function(options) {
		 ...
		 */
		var txtArea = "  <div id='outer'> <input type='text' placeholder='TAG' id='inputTag'/><div id='inner'></div>  </div>";
		//var txtArea = "  <div id='outer'> <input type='text' placeholder='TAG' id='inputTag'/><div id='inner'></div>  </div><button id='doneEdit'>Done</button>";

		/*
		 * alert template
		 */
		var alertClasslate = "<div class='dim' style='display:none;'></div><span id ='alertID' class='alertClass'></span>";
		...
		function createTag() {
			...
			$('#inner').prepend('<span class="spanTag">' + getData + '<img class="tagDelete" alt="Delete Tag" src="images/content/btn_tagDelete.png"/></span>');
		...
		$('#inner').on({
			'mouseover' : function() {
				$(this).find('.tagDelete').prop('src', 'images/content/btn_tagDelete_hover.png');
			},

			'mouseout' : function() {
				$(this).find('.tagDelete').prop('src', 'images/content/btn_tagDelete.png');
			}
		}, '.spanTag');
		 ...
		 */
		if (count == 0) {
			$('#inner').on('click', '.tagDelete', function() {
		...
		$('#doneEdit').on('click', function() {
			 ...
			 */
			if ($(this).text() === 'Done') {
				$('#inner .tagDelete').css('display', 'none');
				$('#inputTag').css('visibility', 'hidden');
				$('#inner').off('click', '.tagDelete');
				$(this).text('Edit').css({
					'background-color' : '#84241a'
				});

			} else {
				$('#inner .tagDelete').css('display', 'inline');
	...
	$.fn.TagTagger.defaults = {
		...
		'duplicateMsg' : {
			'showMsg' : 'true',
			'message' : '<strong>Tag Already Exists</strong><p>동일한 키워드의 태그가 이미 존재합니다.<br />태그 등록을 다시 시도하세요.</p>',
			'bgColor' : '#e3e18a',
			'fontColor' : '#ff0000'
		},
		'tagPool' : {
